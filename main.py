...
from util import flow2rgb, AverageMeter, save_checkpoint, check_reconstruced_imgs
...
def validate(val_loader, model, epoch, output_writers):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        if i < len(output_writers):  # log first output of first batches
            ...
            if epoch == 0:
                mean_values = torch.tensor([0.45,0.432,0.411], dtype=input.dtype).view(3,1,1)
                output_writers[i].add_image('GroundTruth', check_reconstruced_imgs((input[0,:3].cpu() +mean_values)*255, args.div_flow * target[0]), 0)
                output_writers[i].add_image('Inputs', (input[0,:3].cpu() + mean_values).clamp(0,1), 0)
                output_writers[i].add_image('Inputs', (input[0,3:].cpu() + mean_values).clamp(0,1), 1)
            output_writers[i].add_image('FlowNet Outputs',check_reconstruced_imgs((input[0,:3].cpu() +mean_values)*255,args.div_flow * output[0]), epoch)
